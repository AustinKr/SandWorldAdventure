...
namespace SWAEngine::Component
{
	...
	public:
		...
		Tilemap::Tile GetActiveTile(Math::Vector2Int position);
		// Returns a copy of the pending tile or the currently active tile (pending prioritized)
		Tilemap::Tile GetTile(Math::Vector2Int position, int* containerID = nullptr);
		...
		Tilemap::Tile SetTile(Math::Vector2Int position, Tilemap::Tile tile);
		// Swaps the tiles.
		...
		std::pair<Math::Vector2Int, Tilemap::Tile> TryStepMoveTile(Math::Vector2Int origin, Math::Vector2 movement, int maxSteps = MAX_MOVE_STEPS);
	...
	private:
		...
		typedef std::unordered_map<Math::Vector2Int, Tilemap::Tile, Math::Vector2Hasher> TILES;

		// Contains currently active tiles
		Tilemap::ITilemapContainer* mp_ActiveTilesContainer;
		// Contains new override tiles or null tiles to replace any currently active in their place
		Tilemap::ITilemapContainer* mp_PendingTilesContainer;
